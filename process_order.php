...
try {
    ...
    $stmt = $conn->prepare('INSERT INTO orders (firstName, lastName, email, phone, address, sub_total, grand_total, pmode, note, order_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    if ($stmt === false) {
     throw new Exception('Failed to prepare order insertion statement: ' . $conn->error);
    }
$stmt->bind_param('sssssddsss', $firstName, $lastName, $email, $contact, $address, $subtotal, $total, $paymentMode, $orderNote, $defaultStatus);
$stmt->execute();
